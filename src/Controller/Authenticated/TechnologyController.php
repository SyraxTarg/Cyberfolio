...
final class TechnologyController extends AbstractController
{
    ...
    #[Route('/new/{projectId}', name: 'app_technology_new_from_project', methods: ['GET', 'POST'])]
    public function newFromProfile(int $projectId, Request $request, EntityManagerInterface $entityManager): Response
    {
        $technology = new Technology();
        $form = $this->createForm(TechnologyType::class, $technology);
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($file) {
                ...
                $file->move('./images/techLogos', $newFileName);
            } else{
            ...
            return $this->redirectToRoute('app_project_edit', ['id' => $projectId], Response::HTTP_SEE_OTHER);
        }
